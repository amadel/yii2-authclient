...
下一步是向 Web 控制器中添加 [[yii\custom\authclient\AuthAction]]，然后实现 `successCallback` 方法，
...
尽管每个客户端不尽相同，但它们都共享同一个基础接口 [[yii\custom\authclient\ClientInterface]]，
该接口包含了通用的 API。
...
- [[yii\custom\authclient\OpenId]]: 同时定义 `requiredAttributes` 和 `optionalAttributes`.
- [[yii\custom\authclient\OAuth1]] 和 [[yii\custom\authclient\OAuth2]]: 定义 `scope` 域。注意，
  不同的提供商对于范围的格式定义可能不同。

> 提示：如果你正在使用若干个不同的客户端，你可以使用 [[yii\custom\authclient\BaseClient::normalizeUserAttributeMap]] 统一返回属性的列表。

### 通过补充 API 调用获取额外数据

[[yii\custom\authclient\OAuth1]] 和 [[yii\custom\authclient\OAuth2]] 均提供了 `api()` 方法，
可以用于访问外部服务提供商的 REST API。然而该方法比较基础，可能并不足以访问
所有外部 API 功能。该方法主要用于取回外部用户账户数据。

要使用 API 调用，你需要根据 API 说明设置 [[yii\custom\authclient\BaseOAuth::apiBaseUrl]]。
之后就可以调用 [[yii\custom\authclient\BaseOAuth::api()]] 方法了：
...
[[yii\custom\authclient\widgets\AuthChoice]] 小部件用于视图中：
