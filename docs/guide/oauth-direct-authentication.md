...
You may authenticate user via this work flow using [[\yii\custom\authclient\OAuth2::authenticateUser()]].
For example:
...
You may authenticate client only via this work flow using [[\yii\custom\authclient\OAuth2::authenticateClient()]].
For example:
